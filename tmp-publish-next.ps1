...
$dir = 'podcasts/llm-podcast-generator-review/generated/execution/applied'
foreach ($s in $slugs) {
  $cand = Join-Path $dir "$s.candidate.json"
  if (-not (Test-Path $cand)) { Write-Host "MISSING: $cand"; continue }
  Write-Host "=== publish $s ==="
  node podcasts/llm-podcast-generator-review/src/publish-candidate.js --slug $s --candidate $cand --write-live
}
