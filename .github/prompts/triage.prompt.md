...
3. Search for open issues created by or assigned to the user with #tool:mcp_github_github_search_issues. Apply any scope filter provided.
4. For each issue, fetch full details with #tool:mcp_github_github_issue_read to get comment counts, labels, reactions, and recent activity.
5. Gather enhanced data:
...
6. Score each issue using the enhanced priority system:
...
7. Create BOTH triage dashboard documents:
...
8. Confirm: _"Triage dashboard saved to `{md-path}` and `{html-path}` with {N} issues. {X} need immediate action, {Y} are popular with the community."_
