...
1. Fork the `accessibility-agents` repository on GitHub.
2. Select the **Code** button and choose the **Codespaces** tab.
3. Once the environment loads, creating a new folder and a `.agent.md` file using the explorer.
4. Add your agent's instructions, ensuring they follow the schema Aria uses.
5. Create a new branch, commit, and submit a Pull Request to the original repository.
...
3. Create a new branch for your agent: `feature/new-agent-name`.
4. Create a folder under `agents/` and a new file inside: `my-agent.agent.md`. Use the arrow keys in the Explorer to focus the parent folder.
...
1. Fork and clone in one step: `gh repo fork Community-Access/accessibility-agents --clone`.
2. Create a new branch: `git checkout -b feature/new-agent-name`.
3. Create the agent folder and file: `mkdir agents/my-agent` then `touch agents/my-agent/my-agent.agent.md`.
4. Edit the file, stage, and commit: `git add .` then `git commit -m "Create new agent colleague for Aria"`.
