...
.DESCRIPTION
    ...
    - Sets CLASSROOM_ORG_ADMIN_TOKEN repository secret
    - Sets CLASSROOM_ORG, CLASSROOM_DAY1_ASSIGNMENT_URL, CLASSROOM_DAY2_ASSIGNMENT_URL variables
    ...
    One step MUST be completed manually before running this script:
    1. Generate a classic PAT at https://github.com/settings/tokens with the admin:org scope.
       Pass the token value to -AdminPAT.
...
.PARAMETER AdminPAT
    The classic GitHub personal access token with admin:org scope generated by accesswatch.
    This will be stored as the CLASSROOM_ORG_ADMIN_TOKEN repository secret.
...
param(
    ...
    [ValidatePattern('^ghp_[A-Za-z0-9]+$')]
    [string]$AdminPAT,
...
# ---------------------------------------------------------------------------
Write-Step "Setting CLASSROOM_ORG_ADMIN_TOKEN secret on $Repo"
$AdminPAT | gh secret set CLASSROOM_ORG_ADMIN_TOKEN -R $Repo --body -
if ($LASTEXITCODE -eq 0) {
    Write-Pass "CLASSROOM_ORG_ADMIN_TOKEN set"
} else {
    Write-Fail "Failed to set CLASSROOM_ORG_ADMIN_TOKEN"
    $failures.Add("CLASSROOM_ORG_ADMIN_TOKEN secret")
}
...
$variablesToSet = @(
    @{ Name = 'CLASSROOM_ORG';               Value = $ClassroomOrg },
