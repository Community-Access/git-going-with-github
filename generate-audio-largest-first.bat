...
set "STATE_FILE=podcasts\logs\audio-size-order-largest-state.txt"
...
set "LIST_FILE=%TEMP%\audio-size-order-largest-%RANDOM%%RANDOM%.txt"
if exist "%LIST_FILE%" del /q "%LIST_FILE%"

echo.
echo ============================================================
echo  Audio Generator - Largest to Smallest
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; Get-ChildItem '.\podcasts\scripts' -Recurse -File -Filter '*.txt' | Where-Object { $_.BaseName -like 'ep*' -or $_.BaseName -like 'cc-*' } | Group-Object Name | ForEach-Object { $_.Group | Sort-Object Length,FullName | Select-Object -First 1 } | Sort-Object Length,Name -Descending | ForEach-Object { $_.FullName }" > "%LIST_FILE%"
if errorlevel 1 (
